...
#     focus_R = '700'
#     if small_dataset:
#         right_train_dir = '/media/yotamg/bd0eccc9-4cd5-414c-b764-c5a7890f9785/Yotam/Stereo/Tau_right_images/right_images_filtered_small/dn700/'
#         right_test_dir = '/media/yotamg/bd0eccc9-4cd5-414c-b764-c5a7890f9785/Yotam/Stereo/Tau_right_images/right_images_filtered_small/dn700/val/'
#         left_dir = '/media/yotamg/bd0eccc9-4cd5-414c-b764-c5a7890f9785/Yotam/Stereo/Tau_left_images/filtered_dn1500_small/rgb/'
#     else:
...
right_imgs_dir = '/media/yotamg/Yotam/Stereo/Indoor/rectified/Right'
left_imgs_dir = '/media/yotamg/Yotam/Stereo/Indoor/rectified/Left'


right_train_filelist = [os.path.join(right_imgs_dir, img) for img in os.listdir(right_imgs_dir) if img.endswith('.png') or img.endswith('.tif') or img.endswith('.bmp')]
left_train_filelist = [img.replace(right_imgs_dir, left_imgs_dir).replace('R', 'L') for img in right_train_filelist]

right_test_filelist = [os.path.join(right_imgs_dir, img) for img in os.listdir(right_imgs_dir) if img.endswith('.png') or img.endswith('.tif') or img.endswith('.bmp')]
...
angle = 4
x_translation = 0
y_translation = 0.0 / patch_size
scale = 1.0
x_perspective = -0.2
y_perspective = -0.2


train_loader = torch.utils.data.DataLoader(myImageloader(left_img_files=left_train_filelist, right_img_files=right_train_filelist, supervised=False,
                                                         angle=angle,
                                                        ...
                                                        label_transform=transforms.Compose(
                                                         ...
                                                         shuffle=True,
                                                         num_workers=4)
test_db = myImageloader(left_img_files=left_test_filelist, right_img_files=right_test_filelist, supervised=False,
                                                         angle=angle,
                                                         x_translation=x_translation,
                                                         y_translation=y_translation,
                                                         x_perspective=x_perspective,
...
def train(epoch):
    ...
    for batch_idx, (left, right, small_left, small_right) in enumerate(train_loader):
        ...
        # stereo_out *= (torch.mean(mono_out[mono_mask]) / torch.mean(stereo_out[mono_mask]))
        stereo_out *= (torch.mean(mono_out) / torch.mean(stereo_out))

        mono_mask = (stereo_out > 0.5) & (stereo_out < 1.5)
    ...
    if show_images:
        ...
        plt.subplot(234)
        plt.imshow(stereo_out[0].detach().cpu(), vmin=mono_stereo_min, vmax=mono_stereo_max, cmap='jet')
        plt.subplot(235)
        plt.imshow(mono_out[0].cpu(), vmin=mono_stereo_min, vmax=mono_stereo_max, cmap='jet')
        plt.subplot(236)
        plt.imshow((mono_out * mask.float())[0].cpu(), vmin=mono_stereo_min, vmax=mono_stereo_max, cmap='jet')
        plt.show()
...
def test():
    with torch.no_grad():
        model.eval()
        test_loss = 0
        for left, right, small_left, small_right in test_loader:
            left, right, small_left, small_right = left.to(device), right.to(device), small_left.to(device), small_right.to(device)
            stereo_out,theta, right_transformed = model(small_left, small_right)
            with torch.no_grad():
                mono_out, _ = dfd_net(left, focal_point=1.5)
...
def visualize_stn():
    with torch.no_grad():
        ...
        left, right = next(iter(test_loader))
