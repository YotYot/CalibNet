...
class ones_vector(nn.Module):
    def __init__(self, len):
        super(ones_vector, self).__init__()
        self.disp = Variable(torch.Tensor(np.reshape(np.array(np.ones(len)),[1,len,1,1])).cuda(), requires_grad=False)
