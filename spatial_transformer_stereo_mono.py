...
import time
from configurable_stn import ConfigNet
from stn import Net
import math
...
train_labels = [img.replace(right_train_dir, label_dir).replace('_R','').replace('_1500_maskImg','').replace('.png', '.dpt').replace('.tif', '.dpt') for img in right_train_filelist]
test_labels  = [img.replace(right_test_dir,  label_dir).replace('_R','').replace('_1500_maskImg','').replace('.png', '.dpt').replace('.tif', '.dpt') for img in right_test_filelist]
